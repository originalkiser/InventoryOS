...
- **`DeckChart.tsx`** renders the grid's own cells straight into a dark-navy-card `recharts` chart (stacked bar / grouped bar / line / bar+line combo, chosen per slide in `ProcurementDeckPage.tsx`) styled to match the deck's own dark chart cards — brand tokens only (navy background, the palette + the 3 allowed exception colors cycled per series), no new hex.
