...
- **Staffing Report** (`src/modules/customers/StaffingReportPage.tsx`, route `/staffing-report`, Droptop sidebar section) reads `inventory.droptop_time_records` + `inventory.droptop_orders` directly (no dedicated rollup table) and joins them client-side by (location_id, date). Three tabs, one shared Region/Market/AM/Shop + period filter bar above them (same filter shape as Droptop Orders, minus its order-specific Package/Product ID/Vehicle/Fleet dropdowns):
  - **Rollup** — 3-level drill-down for the selected period (shop -> day -> employee timecard), hand-rolled rather than `DataTable`/`useTable` since that shape doesn't fit any of the 5 templates in `TABLE_TEMPLATES.md` (not a flat list, not a matrix) — same precedent as Templates 4/5 for shapes without a shared component yet.
  - **Summary** — company-wide (or filtered) KPIs: orders, labor $, labor % of revenue, labor hours per effective car (LHCE), and labor hours broken out by day-of-week with a count of how many of that weekday occurred in the period (e.g. "Friday (3)"). **"Effective car" = Finalized orders only** (excludes Void/Uncollectible) — matches how Droptop Orders/Customer Heatmap already treat revenue; this is also why the Rollup tab's own Orders column counts Finalized only, not every order regardless of status.
  - **Labor Hour Forecast** — upload a per-(shop, date) forecast (`inventory.labor_hour_forecast`, migration `20260930_labor_hour_forecast.sql`; upload matches header names loosely via regex, not an exact-name mapper) and compare against actual hours as a percent, split hourly-employee vs shop-manager on both sides.
  - **The hourly-vs-manager split is a wage-threshold PROXY, not real data** — confirmed by scanning every distinct key ever present across `droptop_time_records.raw`'s payload: there is no role/title/employee-type field anywhere in what Droptop's time clock API returns, on the user OR the time-record object. The threshold ($/hr and above = manager) is a company-wide setting (`useAppSetting`, key `staffing_manager_wage_threshold`, adjustable on the Forecast tab) precisely because it's approximate — don't treat it as authoritative if a real roster/HR data source ever becomes available.
