...
- **Staffing Report** (`src/modules/customers/StaffingReportPage.tsx`, route `/staffing-report`, Droptop sidebar section) reads `inventory.droptop_time_records` + `inventory.droptop_orders` directly (no dedicated report table) and joins them client-side by (location_id, date) — one row per shop/day: staff headcount (distinct `droptop_user_id`), total hours, order count, orders/labor-hour. Template 1 (Data Table) per `TABLE_TEMPLATES.md`.
