...
export function MonthEndPage() {
  ...
  async function confirmImport() {
    ...
    const { error } = await supabase.from('monthly_counts').insert(rows as any)
    if (error) toast.error(error.message)
    else toast.success(`Imported ${rows.length} month-end count rows`)
    setImporting(false)
    setParsed(null)
    setMappings(null)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-xs text-gray-500 mt-0.5">Upload and manage monthly count data</p>
      </div>

      <div className="grid grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <span className="text-xs font-mono text-gray-400 uppercase tracking-wide">Upload Count File</span>
          </CardHeader>
          <CardBody className="flex flex-col gap-4">
            {!parsed ? (
              ...
              <ColumnMapper
                ...
                onConfirm={(m) => { setMappings(m); setConfirmOpen(true) }}
                onCancel={() => setParsed(null)}
              />
            ) : (
              <div className="text-xs text-[#39ff14] font-mono">
                ✓ {parsed.totalRowsParsed} rows ready to import
              </div>
            )}
          </CardBody>
        </Card>

        <DataSourceLinker configType="monthly_counts" />
      </div>

      <Modal open={confirmOpen} onClose={() => setConfirmOpen(false)} title="Confirm Import">
        <p className="text-sm text-gray-300 mb-4 font-mono">
          Import {parsed?.totalRowsParsed} month-end count records?
        </p>
