...
export function NewPlanModal({ locations, filterMonth, filterYear, existingPlanLocationIds, onClose, onCreated }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 overflow-y-auto py-8">
      <div className="bg-cream rounded-lg shadow-xl w-full max-w-2xl mx-4 flex flex-col gap-5 p-6">
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            ...
            <select className="w-full border border-sky/30 rounded px-3 py-2 text-sm font-mono bg-white"
        ...
        <div>
          ...
          <input className="w-full border border-sky/30 rounded px-3 py-1.5 text-xs font-mono bg-white mb-2"
            placeholder="Search shops…" value={locSearch} onChange={e => setLocSearch(e.target.value)} />
          <div className="border border-sky/20 rounded max-h-40 overflow-y-auto">
            {available.map(loc => {
              ...
              return (
                <label key={loc.id} className="flex items-center gap-2 px-3 py-1.5 hover:bg-sky/10 cursor-pointer text-xs font-mono">
                  ...
                  {loc.location_code && <span className="text-inky/40">{loc.location_code}</span>}
            ...
            <div className="border border-sky/20 rounded max-h-40 overflow-y-auto">
              {templates.map(tpl => (
                <label key={tpl.id} className="flex items-center gap-2 px-3 py-1.5 hover:bg-sky/10 cursor-pointer text-xs font-mono">
                  ...
                  <span className="text-inky/40">{tpl.category}</span>
                  <span className="text-inky/30">· {tpl.campaign_template_tasks?.length ?? 0} tasks</span>
                </label>
              ))}
            </div>
          )}
