...
export default function DepartmentCompletion({ reports, entriesByReport }: Props) {
  ...
  const byDept = useMemo(() => {
    ...
    for (const r of reports) {
      const key = r.department?.name ?? 'Uncategorized'
  ...
  return (
    <div className="pb-6">
      {byDept.map(([deptName, deptReports]) => (
        <div key={deptName}>
          <div className="px-6 pt-6 pb-3 border-b border-sb-inky/20 mb-4">
            <h2 className="font-brand font-bold text-sb-cream text-[20px] tracking-wider uppercase">
              {deptName}
            </h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 px-6 pb-2">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {overdue > 0 && (
                      <span className="flex items-center gap-1 bg-sb-red/20 text-sb-red font-mono text-[11px] px-2 py-0.5 rounded ml-2 flex-shrink-0">
