...
const ICONS: Record<string, JSX.Element> = {
  ...
  integrations: <Icon d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />,
...
function NavItemLink({
  ...
  return (
    <div className="flex items-center gap-1" ref={dragRef} style={dragStyle}>
      ...
      >
        <span className="group-hover/row:hidden">{ICONS[item.key] ?? ICONS.dashboard}</span>
        <span className="hidden group-hover/row:block">{ICONS[item.key] ?? ICONS.dashboard}</span>
...
function SortableSection({
  ...
  const baseItems = SECTION_ITEMS[sectionKey] ?? []
  const items =
    itemOrder.length > 0
