...
// is a centered modal instead of a right-edge slide-out drawer. Section
// order below is deliberately most-frequently-touched-first (Appearance,
// Notifications, Quick Access) down to rarely-touched-once-set (Daily
// Schedule, EOD, Location Exclusions) per the user's own ask.
...
export function ProfilePanel({ onClose, canNotify, permission, notifPrefs, setNotifPrefs, requestPermission }: ProfilePanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose} aria-hidden="true">
      <div
        className="profile-panel relative w-full max-w-md max-h-[85vh] bg-[#F2F1E6] dark:bg-[#002745] shadow-2xl border border-navy/20 dark:border-[#F2F1E6]/10 rounded-xl flex flex-col"
