  ...
  function th(label: string, key: SortKey) {
    ...
    return (
      <th onClick={() => setSort((s) => ({ key, dir: s.key === key && s.dir === 'asc' ? 'desc' : 'asc' }))}
        className="cursor-pointer px-2 py-1.5 text-left hover:text-gray-200">{label}{active ? (sort.dir === 'asc' ? ' ↑' : ' ↓') : ''}</th>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        {withScaleEditor && <button onClick={() => setEditorOpen(true)} className="rounded border border-[#2a2d3e] px-2 py-1 text-xs font-mono text-gray-400 hover:text-white" title="Flag scale">⚙ Scale</button>}
      </div>

      <div className="overflow-auto rounded border border-[#2a2d3e]" style={{ maxHeight }}>
        <table className="w-full text-xs font-mono">
          <thead className="sticky top-0 bg-[#161820] text-gray-500 uppercase tracking-wide">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={5} className="px-2 py-6 text-center text-gray-600">Loading…</td></tr>
            ) : view.length === 0 ? (
              <tr><td colSpan={5} className="px-2 py-6 text-center text-gray-600">No product usage data</td></tr>
            ) : view.map((r: InventoryRow) => (
              <tr key={r.id} className="border-t border-[#2a2d3e]/50" style={r.flag ? { borderLeft: `3px solid ${FLAG_HEX[r.flag]}` } : undefined}>
                <td className="px-2 py-1 text-gray-300">{r.location_label}</td>
                <td className="px-2 py-1 text-gray-300">{r.product_id}</td>
                <td className="px-2 py-1 text-right text-gray-400">{r.on_hands ?? '—'}</td>
                <td className="px-2 py-1 text-right text-gray-400">{r.daily_usage ?? '—'}</td>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <label className="text-xs font-mono text-gray-400">Low threshold (days): <span className="text-[#00e5ff]">{draft.slider_days}</span></label>
        <input type="range" min={1} max={60} value={draft.slider_days} onChange={(e) => setDraft((d) => ({ ...d, slider_days: Number(e.target.value) }))} className="flex-1 accent-[#00e5ff]" />
      </div>
      {draft.bands.map((b) => (
        <div key={b.color} className="rounded border border-[#2a2d3e] p-2">
          <div className="mb-2 flex items-center gap-2">
            <span className="inline-block h-3 w-3 rounded-full" style={{ background: FLAG_HEX[b.color] }} />
            <span className="text-xs font-mono uppercase text-gray-300">{b.color}</span>
            <button onClick={() => addRange(b.color)} className="ml-auto text-xs font-mono text-[#00e5ff] hover:underline">+ range</button>
          </div>
          {b.ranges.map((r, i) => (
            <div key={i} className="mb-1 flex items-center gap-2 text-xs font-mono text-gray-400">
              <input type="number" placeholder="min" value={r.min ?? ''} onChange={(e) => setBandRange(b.color, i, { min: numOrNull(e.target.value) })}
                className="w-20 rounded border border-[#2a2d3e] bg-[#0f1117] px-2 py-1 text-white" />
              <span>≤ days &lt;</span>
              <input type="number" placeholder="max" value={r.max ?? ''} onChange={(e) => setBandRange(b.color, i, { max: numOrNull(e.target.value) })}
                className="w-20 rounded border border-[#2a2d3e] bg-[#0f1117] px-2 py-1 text-white" />
              <button onClick={() => removeRange(b.color, i)} className="text-gray-600 hover:text-red-400">×</button>
            </div>
          ))}
          <p className="text-[10px] text-gray-600">Blank min/max = unbounded. Multiple ranges = OR (e.g. too-low or too-high).</p>
        </div>
      ))}
