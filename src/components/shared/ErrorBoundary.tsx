    ...
    if (this.state.error) {
      return (
        <div className="flex items-center justify-center py-16 px-6 font-mono">
          <div className="max-w-lg w-full bg-cream border border-red-500/40 rounded-lg p-6 flex flex-col gap-3">
            ...
            <pre className="text-[11px] text-red-300 bg-cream border border-navy/30 rounded p-3 overflow-auto max-h-48 whitespace-pre-wrap">
              {this.state.error.message}
              {this.state.error.stack ? `\n\n${this.state.error.stack}` : ''}
            </pre>
            <div className="flex gap-2">
              <button onClick={() => this.setState({ error: null })}
                className="text-xs font-mono text-inky border border-[#00e5ff]/30 rounded px-3 py-1.5 hover:bg-[#00e5ff]/10">
                Try again
              </button>
              <button onClick={() => window.location.reload()}
                className="text-xs font-mono text-inky border border-navy/30 rounded px-3 py-1.5 hover:border-gray-500">
                Reload page
              </button>
            </div>
          </div>
