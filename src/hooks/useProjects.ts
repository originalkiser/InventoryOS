...
export function useProjects() {
  ...
  useEffect(() => {
    ...
    const ch = supabase
      .channel('projects-rt')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'projects', filter: `company_id=eq.${companyId}` }, () => load())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'project_tasks', filter: `company_id=eq.${companyId}` }, () => load())
      .subscribe()
  ...
  async function reorderProjects(ordered: Project[]) {
    setProjects(ordered)
    const updates = ordered.map((p, i) => sb.from('projects').update({ sort_order: i }).eq('id', p.id))
