...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (whoErr || !who.user) return json({ error: 'Not authenticated' }, 401)

    const { data: me, error: meErr } = await caller
      .from('profiles').select('company_id, role, email').eq('id', who.user.id).single()
    if (meErr || !me) return json({ error: 'Your profile was not found' }, 403)
    if (me.role !== 'admin') return json({ error: 'Only admins can invite users' }, 403)
    ...
    if (!email || !fullName || !password) return json({ error: 'Name, email, and password are required' }, 400)
    ...
    if (adminDomain && newDomain && adminDomain !== newDomain) {
      return json({ error: `Email must be on the @${adminDomain} domain` }, 400)
    }
    ...
    if (createErr || !created.user) return json({ error: createErr?.message ?? 'Could not create the user' }, 400)
